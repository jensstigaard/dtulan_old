...
class UploadBehavior extends ModelBehavior {
	...
	public function beforeSave(Model $model) {
		...
		foreach ($this->settings[$model->alias] as $field => $options) {
			...
			$fileName = explode('.', $this->runtime[$model->alias][$field]['name']);
	...
	public function afterSave(Model $model, $created) {
		...
		foreach ($this->settings[$model->alias] as $field => $options) {
			...
			$fileName = explode('.', $model->data[$model->alias][$field]);
			$ext = strtolower(end($fileName));

			$model->data[$model->alias][$field] = $model->data[$model->alias]['id'] . '.' . $ext;

			$filePath = $path . $model->data[$model->alias][$field];

			$this->log('FilePath: ' . $filePath, 'upload');
			$this->log('Filename: ' . $model->data[$model->alias][$field], 'upload');
	...
	public function _resizeImagick(&$model, $field, $path, $size, $geometry, $thumbnailPath) {
		...
		$fileName = str_replace(
				  array('{size}', '{filename}', '{primaryKey}'), array($size, $pathInfo['filename'], $model->id), $this->settings[$model->alias][$field]['thumbnailName']
		);
	...
	public function _prepareFilesForDeletion(&$model, $field, $data, $options) {
		...
		foreach ($options['thumbnailSizes'] as $size => $geometry) {
			$fileName = str_replace(
					  array('{size}', '{filename}', '{primaryKey}'), array($size, $pathInfo['filename'], $model->id), $options['thumbnailName']
			);
