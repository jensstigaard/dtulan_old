...
class PizzaOrdersController extends AppController {
    ...
    public function beforeFilter() {
        parent::beforeFilter();
        $this->Auth->allow('api_add', 'api_view', 'api_edit');
    }
        ...
        if (!$this->PizzaOrder->PizzaWave->isOrderable($data['wave_id'], $this->isAdmin())) {
            ...
            if (isset($this->request->data['PizzaOrderItem']) && count($this->request->data['PizzaOrderItem'])) {
                ...
                if ($this->PizzaOrder->saveAssociated($this->request->data)) {
                    $msg = 'SUCCESS';
                } elseif (isset($this->PizzaOrder->validationErrors['User']['balance'][0])) {
        ...
        if ($this->request->is('put')) {
            if ($this->isJsonRequest()) {
                ...
                if ($this->PizzaOrder->exists()) {
                    ...
                    if(isset($this->request->data['PizzaOrder']['status'])) {
                        $this->PizzaOrder->data['PizzaOrder']['status'] = intval($this->request->data['PizzaOrder']['status']);
                    }
                    if ($this->PizzaOrder->save()) {
                        $this->set('success', true);
                        $this->set('data', array('message' => __('Pizza order status: ' . $this->PizzaOrder->data['PizzaOrder']['status'])));
                    } else {
...
//            if ($this->isJsonRequest()) {
                if (isset($this->params['named']['user'])) {
                    ...
                    $pizza_orders = $this->PizzaOrder->getPizzaOrdersByUser($user_id);
                    $this->set('success', true);
                    $this->set('data', array('pizza_orders' => $pizza_orders));
                } else {
                    $this->PizzaOrder->id = $id;
                    if ($this->Pizzaorder->exists()) {
                        $this->PizzaOrder->read();
